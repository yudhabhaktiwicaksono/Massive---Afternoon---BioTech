...
const Login = () => {
    ...
    return (
      <div>
        ...
        <div className='item-login-selected'>
          <button className='my-3 item-selected' onClick={() => setContentRegister('content')}>
  ...
  const LoginContent = () => {
    return (
      <div className='w-100 p-5 flex-column d-flex justify-content-center' style={{ height: window.innerHeight }}>
        <div>
          ...
          <div className={`col-8 content-login-${contentRegister}`}>
            {
              contentRegister === 'selected' ? SelectedContent() :
                contentRegister === 'content' ? LoginContent() : ''
            }
          </div>
        </div>
      </div >
